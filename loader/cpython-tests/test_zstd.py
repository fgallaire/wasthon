...
def setUpModule():
    ...
    # Brython can't os.path.abspath(__file__) — it's a URL, not a path.
    # The tests that consume THIS_FILE_BYTES just need *some* reasonably-
    # sized text bytes to compress/decompress, so substitute a synthetic
    # payload.
    try:
        ...
        THIS_FILE_BYTES = (b'# synthetic test_zstd payload\n'
                           b'lorem ipsum dolor sit amet ' * 4000)
        THIS_FILE_STR = THIS_FILE_BYTES.decode('utf-8')
...
class ZstdDictTestCase(unittest.TestCase):
    ...
    def test_invalid_dict(self):
        # Brython 3.14.1 parser bug: chokes on `0xEC30A437.to_bytes(...)`
        # — interprets the `.` as a decimal separator on the hex literal.
        # Parenthesizing the int literal works around it.
