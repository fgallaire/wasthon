/*
   Copied (near-)verbatim from CPython:
     Include/internal/pycore_blocks_output_buffer.h
   Copyright (C) 2001-present Python Software Foundation.
   Licensed under the Python Software Foundation License v2.
   Full text: https://docs.python.org/3/license.html
   The only Wasthon-side change is a relaxed Py_BUILD_CORE guard so the
   header compiles under our build flags.
...
#if !defined(Py_BUILD_CORE) && !defined(Py_BUILD_CORE_MODULE) && !defined(Py_BUILD_CORE_BUILTIN)
